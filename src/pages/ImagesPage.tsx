...
export default function ImagesPage() {
    return (<div style={{ padding: "var(--space-6)", maxWidth: 1200 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "var(--space-5)" }}>
            ...
            <button className="btn btn-primary btn-sm">Pull Image</button>
        </div>
                ...
                <tbody>
                    {images.map(img => (
                        <tr key={img.id} style={{ borderBottom: "1px solid var(--color-border)" }}>
                            ...
                            <td style={{ padding: "10px 16px" }}><span style={{ background: "var(--color-bg-tertiary)", padding: "2px 8px", borderRadius: 4, fontFamily: "var(--font-mono)", fontSize: "12px" }}>{img.tag}</span></td>
                            ...
                            <td style={{ padding: "10px 16px" }}><button className="btn btn-sm" style={{ color: "var(--color-danger)" }}><Trash2 size={12} /> Remove</button></td>
                        </tr>
                    ))}
                </tbody>
            </table>
